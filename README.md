# OTPFloodGuard Benchmark

This repository contains the code and generated files used for the paper:

**OTPFloodGuard: A Public-Evidence-Constrained Benchmark for Lightweight OTP Flooding Detection**

Authors: Wenche An and Kamran Aziz

## What is included

- `src/run_otpfloodguard_experiment.py`  
  Python script used to generate the simulated benchmark, train baseline models, run stress tests, and export the result files.
- `manuscript.md`  
  Current manuscript source used by the PDF build script.
- `data/`  
  Simulated OTP request-window data generated by the script.
- `results/`  
  CSV files used for the paper tables and reported metrics.
- `figures/`  
  Figures used in the paper.
- `OTPFloodGuard_PRAI2026_Manuscript.pdf`  
  Formatted manuscript PDF corresponding to the current repository contents.
- `scripts/check_public_artifacts.py`  
  Local check for accidental absolute paths or obvious secret patterns in public text artifacts.

## Scope

OTPFloodGuard is a simulated benchmark package. It does not contain real OTP logs, real phone numbers, private user records, or production authentication data. Public evidence constrains the direction of risk signals, while numeric distributions and difficulty controls remain benchmark design choices. Reported scores are benchmark results under stated simulation assumptions, not production accuracy claims.

## Run a quick check

From the repository root:

```bash
pip install -r requirements.txt
python3 src/run_otpfloodguard_experiment.py --quick
```

This creates a fast seed-42 quick-verification output in `results/quick/`. Quick verification is not full paper reproduction and does not overwrite the formal result tables in `results/`.

For the exact package versions used to generate the current artifacts, see `requirements-lock.txt` and `results/environment.json`.

## Model configuration

- Random Forest: `n_estimators=250`, `max_depth=10`, `min_samples_leaf=3`, `class_weight=None`.
- Logistic Regression: `StandardScaler` fitted on the training split plus `LogisticRegression(max_iter=1000)` with default L2 regularization.
- Gradient Boosting: full parameters are exported in `results/model_config.json` from scikit-learn `get_params()`.
- Tuned rule baseline: request, failure-rate, prefix-concentration, and repeat-IP thresholds selected by five-fold stratified cross-validation inside the training split only.
- Cost-sensitive thresholds: selected from five-fold training out-of-fold predictions, then evaluated once on the held-out test set.
- Top-k feature subsets: selected with Random Forest impurity-based feature importance computed from the training portion only. Cross-validated permutation importance is exported separately for interpretation and is not used for Top-k feature selection.
- Model scores: used as empirical decision scores for thresholds, not as calibrated attack probabilities.

## Evaluation boundary

The primary experiment uses a stratified 80/20 train-test split. Model fitting, feature ranking, scaling, rule-threshold selection, and cost-sensitive threshold selection use only the training portion. The held-out test set is reserved exclusively for final reporting after all model, feature, scaler, and threshold choices have been fixed.

The main diagnostic model for ROC/PR curves, cost-sensitive thresholds, permutation importance, error analysis, and base-rate sensitivity is fixed as Random Forest with the full feature set before held-out reporting. The held-out test set is not used to select this diagnostic model.

Generator-seed sensitivity evaluates five separately regenerated Overlap benchmark instances with generator seeds `11, 23, 42, 67, 101` while holding the split seed at `42`. It measures sensitivity to generator randomness and does not rerun full diagnostics.

Generator-shift evaluation transforms the held-out test rows with a separately seeded, class-conditional, label-preserving feature generator. Ground-truth labels are used only to construct the synthetic shift and are not provided to the detector. Models, fitted scalers, selected features, model-score thresholds, and rule thresholds are not refitted or retuned on the shifted set.

## Reproduce the paper results

To regenerate the full simulated benchmark results:

```bash
python3 src/run_otpfloodguard_experiment.py
```

This writes the simulated dataset, result CSV files, metadata JSON files, and figures to `data/`, `results/`, and `figures/`.

The full run may take several minutes because it includes multi-split evaluation, generator-seed sensitivity, training-only rule selection, generator-shift testing, attack-intensity analysis, feature-group ablation, threshold analysis, base-rate sensitivity analysis, deterministic error-case selection, training-fold permutation importance, and diagnostic figures.

To build the PDF after full reproduction:

```bash
python3 src/build_ieee_pdf.py
```

`pdflatex` must be available on `PATH`.

To check public artifacts for accidental local paths or obvious secret patterns:

```bash
python3 scripts/check_public_artifacts.py
```

## Paper-facing artifacts

- `results/metrics.csv` -- main Overlap benchmark results
- `results/benchmark_config.json` -- dataset size, class balance, and controlled-prevalence interpretation
- `results/multi_split_summary.csv` -- seven-split stability results
- `results/generator_seed_metrics.csv` -- per-generator-seed model results for five separately regenerated Overlap benchmarks
- `results/generator_seed_summary.csv` -- mean and standard deviation across generator seeds
- `results/cross_generator_metrics.csv` -- generator-shift robustness results
- `results/difficulty_metrics.csv` -- Easy, Overlap, and Adaptive results
- `results/attack_intensity_metrics.csv` -- attack-intensity stress-test results
- `results/ablation_metrics.csv` -- feature-group ablation results
- `results/cost_sensitive_thresholds.csv` -- held-out threshold results under relative FN/FP costs
- `results/prevalence_sensitivity.csv` -- prior-adjusted precision projections under alternative assumed attack prevalences
- `results/threshold_selection_oof.csv` -- training out-of-fold threshold-selection diagnostics
- `results/error_analysis_summary.csv` -- representative false-positive and false-negative patterns
- `results/error_case_selection.json` -- deterministic representative-error-case selection method and selected row IDs
- `results/representative_error_cases.csv` -- selected representative false-positive and false-negative cases
- `results/feature_ranking_metadata.json` -- training-only Top-k ranking method and relation to permutation importance
- `results/permutation_importance_cv.csv` -- permutation importance computed on validation folds within the training portion
- `results/reporting_precision.json` -- manuscript decimal-place and rounding rule
- `results/feature_dictionary.csv` -- feature definitions
- `results/test_predictions.csv` -- seed-42 test predictions used for threshold and error analysis

## Reproduction metadata

- `results/model_config.json` -- full estimator parameters exported from `get_params()`
- `results/rule_config.json` -- rule logic, threshold grids, and selected rule parameters
- `results/environment.json` -- Python and package versions
- `results/generator_config.json` -- generator controls and parameter-provenance notes
- `results/generator_seed_config.json` -- generator-seed sensitivity protocol
- `results/generator_shift_metadata.json` -- generator-shift construction and no-retuning metadata
- `requirements-lock.txt` -- package versions matching the current recorded environment

## Auxiliary diagnostics

- `results/duplicate_audit.csv` and `results/duplicate_audit.json` -- duplicate-row and train-test overlap audit for generated data
- `results/attack_subtype_recall.csv` -- held-out recall by simulated attack subtype for the fixed diagnostic model
- `results/complement_feature_sanity.csv` -- performance sanity check for deterministic complement outcome features
- `results/complement_identity_sanity.csv` -- identity check for success_rate + failure_rate
- `results/quick/` -- quick-verification outputs that do not overwrite formal result tables

## Pending regeneration

Some result files and figures still correspond to the previous full reproduction and will be regenerated once after all source and logic revisions are complete. Legacy window-scaling artifacts are no longer part of the formal paper-facing artifact set.

## Notes

The data in this repository is simulated. It does not contain real OTP logs, real phone numbers, private user records, or production authentication data.

The reported scores should be read as benchmark results under the stated simulation assumptions, not as real-world OTP flooding detection accuracy.
